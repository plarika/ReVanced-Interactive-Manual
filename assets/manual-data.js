window.MANUAL_DATA = {
  ...
  "sources": [
    ...
    {
      "id": "R23",
      "name": "ReVanced Manager issue #3471 — Android 8.0: patch termina mas instalação/exportação não fica disponível",
  ...
  "currentReports": [
    ...
    },
    {
      ...
      "summary": "A issue oficial inclui logs com respostas 400 Bad Request em ANDROID_UNPLUGGED, ANDROID_CREATOR, IOS_UNPLUGGED e ANDROID_VR_AUTH, terminando em Could not fetch any client streams. Vários utilizadores reportaram falhas semelhantes. Android VR ou visionOS resolvem alguns casos, mas falham noutros. Um relato isolado indica que desativar Spoof Video Streams fez o playback voltar a funcionar mesmo com Android VR já selecionado. Em 2026-08-26, outro utilizador relatou que trocar clientes, reiniciar, repatch e rollback para versões antigas não resolveu de forma duradoura; o repatch ajudou apenas cerca de 24 horas antes da regressão. Estes comportamentos são contraditórios e não estão confirmados pelo upstream; não devem ser tratados como correção geral. Ainda não existe confirmação upstream da causa nem fix oficial.",
      "action": "Confirmar versão do YouTube, patches e ReVanced Manager; guardar logs antes de alterar definições. Testar apenas um cliente de Spoof video streams de cada vez e reiniciar totalmente a app entre testes. Android VR e visionOS podem ser testados como workarounds relatados, mas não são correções confirmadas. Desativar Spoof Video Streams só deve ser considerado como teste diagnóstico temporário para reproduzir o relato isolado, com reversão imediata se não resolver ou degradar playback; não recomendar essa opção como configuração permanente sem confirmação upstream. Se repatch ou rollback produzir melhoria apenas temporária, registar quanto tempo dura e reverter ao baseline suportado em vez de repetir ciclos de downgrade. Se todos os clientes devolverem 400, não limpar conta nem reinstalar GmsCore por defeito; aguardar confirmação upstream ou patches atualizados.",
    ...
    },
    {
      ...
      "summary": "A issue oficial reúne múltiplas confirmações de vídeos que demoram cerca de 1–2 minutos a iniciar e, nalguns casos, ficam presos em baixa qualidade ou sem seletor de qualidade. Relatos adicionais na issue #3508 descrevem reprodução inicialmente em HD, seguida após cerca de 1 minuto por buffering/ecrã preto e regressão para 240p/480p, por vezes com o seletor de qualidade indisponível. Vários utilizadores relatam melhoria ao mudar o Default client de Spoof video streams para visionOS; Android VR também funciona em alguns casos. Há, porém, relatos em que estes clientes deixam de resolver ou não resolvem de todo. Uma corroboração recente na #3508, com patches 6.2.1 e Manager 2.6.0, relata qualidade presa perto de 360p em parte dos vídeos e ausência de melhoria ao testar Android Reel, Android Studio, visionOS e outros clientes. Outro utilizador relatou melhoria ao alterar simultaneamente para visionOS e ativar Force AVC; como duas variáveis foram mudadas ao mesmo tempo, esse relato inicial não isolou qual delas teve efeito. Em 2026-09-10, o autor da #3508, que antes tinha relatado ausência de melhoria ao testar vários clientes, voltou a indicar especificamente que Force AVC lhe resolveu o bloqueio de qualidade. Isto reforça Force AVC como workaround comunitário relatado, mas o histórico de mudanças anteriores impede tratá-lo como teste perfeitamente isolado ou correção universal. Não existe correção universal confirmada.",
      "action": "Confirmar versão do YouTube e patches, ativar debug logs e testar o mesmo vídeo com uma variável de cada vez. Se disponível, testar visionOS ou Android VR apenas como workaround relatado, reiniciando totalmente a app entre testes. Comparar tempo até iniciar, qualidade e seletor. Um relato recente na issue #3467 indica que visionOS pode impedir a reprodução de vídeos 360/VR; se esse conteúdo for relevante, incluir um vídeo 360/VR no teste A/B e reverter o cliente se houver regressão. Se o problema persistir em vários clientes, voltar ao baseline e, se Force AVC estiver disponível, testá-lo separadamente como workaround relatado, mantendo o cliente e todas as restantes variáveis constantes; reverter se não melhorar. Preservar logs e não atribuir a falha à rede ou ao GmsCore sem evidência. Um relato isolado na #3508 associa bloqueio de qualidade a ecrã preto e reboot do dispositivo, sem causa identificada; se ocorrer, interromper testes repetitivos, preservar logcat e logs de sistema/vendor quando disponíveis e regressar ao baseline, sem assumir causalidade com ReVanced.",
    ...
    {
      "issue": 3510,
      "title": "Restore old version playback speed menu pode não restaurar a interface antiga",
      "state": "OPEN",
      "reported": "2026-08-27",
      "classification": "RELATADO / NÃO CONFIRMADO",
      "evidence": "Baixa",
    ...
    },
    {
      ...
      "summary": "A issue oficial #3511 relata que a reprodução no YouTube Music pode começar normalmente e, após cerca de 1 minuto, entrar em erro/buffering e deixar de continuar; pelo menos dois utilizadores adicionais confirmaram comportamento semelhante. O debug log do autor mostra Spoof video streams a usar ANDROID_REEL_NO_AUTH. Um utilizador reportou melhoria temporária ao mudar o Default client para visionOS, mas não existe confirmação upstream de causa nem de correção universal. A issue oficial ReVanced/GmsCore #381, atualizada em 2026-09-05, acrescenta vários relatos independentes do mesmo padrão em YouTube Music e YouTube; três utilizadores reportam que mudar apenas o Default client para Android VR resolveu o bloqueio de ~1 minuto. Porém, um relato posterior de 2026-09-05 na mesma issue indica que Android VR não resolveu noutro dispositivo; o autor associou temporalmente o início do problema a uma atualização do GmsCore/microG, sem evidência suficiente para atribuir causalidade. Isto mantém Android VR como workaround comunitário reproduzido, mas contraditório e não universal. Em 2026-09-06, um comentário posterior na própria issue #3511 indicou que o workaround visionOS deixou de funcionar nesse caso; outro utilizador relatou separadamente \"Could not fetch any client streams\" em conteúdo \"for kids\". Estes dois pontos são relatos isolados e não confirmam uma nova causa, âmbito ou correção.",
      "action": "Registar versões do YouTube Music, Manager e patches e testar o mesmo conteúdo durante pelo menos 3 minutos. Fazer teste A/B mudando apenas o Default client de Spoof video streams, reiniciando totalmente a app entre testes. visionOS deve ser tratado como workaround transitório e contraditório: existe um relato posterior de que deixou de funcionar. Se for testado, manter todas as outras variáveis constantes e voltar ao baseline se não melhorar ou houver regressão. Preservar debug logs e não atribuir o problema à rede, conta ou GmsCore sem evidência adicional. Android VR tem múltiplas confirmações na issue oficial GmsCore #381, mas também pelo menos um relato de falha; pode ser priorizado como teste A/B, nunca como correção, mantendo todas as restantes variáveis constantes.",
    ...
    },
    {
      ...
      "title": "ReVanced Patches: repositório GitHub temporariamente indisponível; desenvolvimento continua no mirror oficial GitLab",
      "state": "CONFIRMADO",
      "reported": "2026-03-24",
      "classification": "CONFIRMADO / OFICIAL",
      "evidence": "Alta",
      "scope": "ReVanced Patches / fontes oficiais / GitHub / GitLab",
    ...
    {
      "issue": 3519,
      "title": "Android 8.0: patch termina com sucesso, mas Save/Install podem ficar desativados",
      "state": "OPEN",
      "reported": "2026-08-25",
      "classification": "RELATADO / MÚLTIPLOS RELATOS",
      "evidence": "Média",
